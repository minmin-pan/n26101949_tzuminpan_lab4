...
class Game:
    ...
    def select(group, x, y):
        ...
        """
        for tower in group.get():               # use to get one tower each loop
            if tower.is_clicked(x, y):          # if click tower
                tower.get_selected(True)        # reassign the variable of is_selected(use to control range show or not)
            else:
